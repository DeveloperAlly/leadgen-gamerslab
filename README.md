...
leadgen-gamerslab/
├── workflow/
│   ├── gamerslab-outreach-v9.json          ← n8n workflow — import this
│   └── gamerslab-outreach-v9.backup.json   ← prior snapshot (23 nodes)
├── supabase/
...
├── Gamers Lab Lead Gen Pipeline/           ← Gen2: white-label productisation campaign (design/governance)
├── .env.example                            ← copy to .env, fill in SERPER_API_KEY
└── README.md
```

> **Two generations live here.** This README covers **Generation 1** — the built, Steam-specific
> outreach pipeline. The `Gamers Lab Lead Gen Pipeline/` folder holds **Generation 2**, a
> separate design/governance campaign to turn this into a modular white-label product. Gen2 is
> research/design-stage and does not run.

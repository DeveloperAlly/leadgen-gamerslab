...
const getDraft = node({
  ...
  config: {
    ...
    parameters: {
      ...
      // Resolves a specific message when the webhook passes message_id (follow-ups), else
      // the publisher's step-1 control. 'none' sentinel because n8n drops a trailing empty param.
      query: "SELECT m.id AS message_id, COALESCE(m.subject, '') AS subj, COALESCE(m.body, '') AS body_text, p.contact_email, m.thread_id FROM public.message m JOIN public.publishers p ON p.id = m.publisher_id WHERE (NULLIF($2, 'none') IS NOT NULL AND m.id = NULLIF($2, 'none')::uuid) OR (NULLIF($2, 'none') IS NULL AND m.publisher_id = $1 AND m.step = 1) ORDER BY (m.variant = 'A') DESC, m.created_at ASC LIMIT 1",
      options: { queryReplacement: expr("{{ $json.body.publisher_id }},{{ $json.body.message_id || 'none' }}") },
    },
...
const gmailSend = node({
  ...
  config: {
    ...
    parameters: {
      ...
      contentType: 'json',
      specifyBody: 'json',
      // Include threadId only when present so the follow-up threads as a reply; step-1 omits it.
      jsonBody: expr('{{ JSON.stringify($json.threadId ? { raw: $json.raw, threadId: $json.threadId } : { raw: $json.raw }) }}'),
    },
  },
});

...
| **Prompts / AI calls** | This *is* the call path — inject the key server-side and forward. **Two v9-proven patterns:** (1) **dynamic free-model discovery** — query OpenRouter for *currently* `:free` models and filter at runtime (the roster shifts); **gate the roster on `context_length` (admit only models ≥ a floor, default 128K) and required features (structured output)** so the always-loaded CAG budget is *deterministic* regardless of which free models exist that day; (2) for quality-critical steps, **fan out the same prompt across several free models and pick the best** response |
...
| **Watch-outs** | Free OpenRouter = **20 req/min, 50 req/day** until a **one-time $10** purchase lifts it to **1,000 req/day** ✅ — design within this for the POC (failed attempts still count against the daily quota ✅). Store keys **encrypted in Vault**, never client-side. ⚠️ Vault is the right per-tenant fit but is "a documented option," not a verbatim "the recommended" pattern — our architectural call. Edge Functions **can't** read Vault via env; they query `vault.decrypted_secrets` over the DB connection ✅. **⭐ "CAG" here = a curated block *re-sent every call*, not KV-cache reuse.** Provider prompt-caching (DeepSeek auto, Gemini 2.5 implicit, Anthropic via `cache_control`) **can't be relied on across a free multi-model fan-out** — so re-sending the block each call is the model, and since `:free` calls are **$0** the binding constraint is the **context-window floor + answer quality**, *not* cache cost. Size the CAG against the **floor (128K) minus per-prospect payload minus output**, and cap it far below that for quality (see L2) ✅ (OpenRouter context windows verified 2026-06-22) |
...
| **Output / data contract** | `business_summary` JSON (value_props[], icp{}, pains[], channels{cold_fit, warm_assets}, where_to_find[]) — **editable**, versioned, `tenant_id`-scoped → **CAG** (**budget: target ≤ ~2–4k tokens; hard ceiling = free-model context floor − per-prospect payload − output**, set by F4's context-length gate) |
| **Free-tier path** | Free models via F4; gte-small retrieval free ✅ |
| **Watch-outs** | Quality of *everything downstream* is set here. The biggest unknown is GamersLab's *real* ICP and whether cold is even its channel — Gate A exists to surface that, not assume it. **Keep the CAG small for *quality*, not just fit** — over-stuffing a big always-loaded block degrades attention ("lost in the middle") and adds latency on the free tier *well before* it threatens the window. The current `gamerslab-poc/docs/cag-block.md` (~1.5–2k tokens) is already well-sized; enforce the ceiling with a **token-count guard in the Business-context editor** (warn past budget) |
